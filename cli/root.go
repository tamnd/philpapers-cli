// Package cli builds the philpapers command tree on top of the philpapers library.
package cli
...
type App struct {
	client *philpapers.Client
	cfg    philpapers.Config
...
func Root() *cobra.Command {
	app := &App{cfg: philpapers.DefaultConfig()}

	root := &cobra.Command{
		Use:           "philpapers",
		Short:         "Browse PhilPapers philosophy research database",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = philpapers.NewClient(a.cfg)
	return nil
}
