...
describes what users get. Pick a format with `-o`, or let philpapers choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
philpapers <command> -o table   # aligned columns for reading
philpapers <command> -o jsonl   # one JSON object per line, for piping
philpapers <command> -o json    # a single JSON array
philpapers <command> -o csv     # spreadsheet friendly
philpapers <command> -o tsv     # tab-separated
philpapers <command> -o url     # just the URL column
philpapers <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
philpapers <command> --fields id,title,url
```
...
```bash
philpapers <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
philpapers <command>            # a table, because this is a terminal
philpapers <command> | wc -l    # JSONL, because this is a pipe
```
